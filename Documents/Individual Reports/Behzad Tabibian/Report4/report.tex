...
\end{figure}
Using equation \ref{equ:getBallWall} rebound position on the wall is calculated so that Figure \ref{fig:bounceWall} holds.
\begin{align}
\label{equ:getBallWall}
{{Y_{ball}-Y_{wall}} \over{X_{ball}-X}} = {{Y_{goal}-Y_{wall}} \over {X_{goal}-X}}\\
X = {{X_{ball}(Y_{goal}-Y_{wall})}\over {Y_{ball} + Y_{goal}-2Y_{wall}}}+\\{ X_{goal}(Y_{ball}-Y_{wall})\over {Y_{ball} + Y_{goal}-2Y_{wall}}}
\end{align}
